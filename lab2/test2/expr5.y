...
expr    : expr ADD expr   { int myresult1 = atoi($1);int myresult2 = atoi($3);char intStr[50];sprintf(intStr, "%d", (myresult1 + myresult2));$$ = intStr; } 
        | expr SUB expr   { int myresult1 = atoi($1);int myresult2 = atoi($3);char intStr[50];sprintf(intStr, "%d", (myresult1 - myresult2));$$ = intStr; } 
        | expr MUL expr   { char *endptr; int myresult1 = strtof($1, endptr);float myresult2 = strtof($3, endptr);char floatStr[50];int maxLength = 50;snprintf(floatStr, maxLength, "%f", (myresult1 * myresult2));$$ = floatStr; }  
        | expr DIV expr   { char *endptr; float myresult1 = strtof($1, endptr);float myresult2 = strtof($3, endptr);char floatStr[50];int maxLength = 50;snprintf(floatStr, maxLength, "%f", (myresult1 / myresult2));$$ = floatStr; }  
        | LEFTP expr RIGHTP    { $$ = $2;  } 
        | '-' expr %prec UMINUS   { char *endptr; float myresult1 = strtof($2, endptr);char floatStr[50];int maxLength = 50;snprintf(floatStr, maxLength, "%f", (-myresult1));$$ = floatStr; }  
        | NUMBER { $$ = strdup($1);}
        | ID {
            int i;
            for (i = 0; i < symbolTableSize; i++) {
